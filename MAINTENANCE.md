...
本项目为 OpenWrt 附加软件包仓库（`openwrt-21.02` 分支，适用于较旧版 OpenWrt/`fw3` 及 `ucode` 解析器环境），包含一些 `-next` 软件源。为保持二进制程序及启动脚本的高可用性，需进行定期常规维护。
...
3. **注意旧版系统环境兼容性**：由于此分支面向 `openwrt-21.02` 旧版本环境，因此在更新配置及相关脚本时，必须确保配置项与老旧的 `fw3` (iptables)、旧版 `netifd` 以及较旧的依赖组件兼容，切勿盲目照搬 `main` 分支的特有规则。
